...
pub fn match_pattern(input_line: &str, pattern: &str) -> Result<bool> {
    for start in 0..input_line.len() {
        ...
        while input_idx < input_line.len() && pattern_idx < pattern.len() {
            ...
            let is_match;
            (is_match, pattern_idx) = match_char(input_char, pattern, pattern_idx)?;
            if !is_match {
                break;
            }
            input_idx += 1;
...
fn match_char(input_char: char, pattern: &str, pattern_idx: usize) -> Result<(bool, usize)> {
    match &pattern[pattern_idx..] {
        p if p.starts_with("\\d") => {
            if !input_char.is_ascii_digit() {
                return Ok((false, pattern_idx));
            }
            Ok((true, pattern_idx + 2))
        }
